...
import SortableWork from './SortableWork';
...
const WorkBuilder = () => {
  ...
  useEffect(() => {
    const jsonItems = data ? JSON.parse(data?.work?.param1) : [];
    setItems(jsonItems);
  }, [data]);
  ...
  const handleSubmit = () => {
    ...
    repeaterUpdate('work', params, setActionLoader);
  };
